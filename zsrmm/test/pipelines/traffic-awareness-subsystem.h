...
void *air_radar_task(void *argp){
  ...
  cpuattr.zs_instant.tv_nsec=600000000;
  ...
  for (i=0;i<10;i++){
    busy_timestamped(100,timestamps_ns9, MAX_TIMESTAMPS,&bufidx9);
    sprintf(buf,"msg[%d]",i);
...
void *object_identification_task(void *argp){
  struct zs_reserve_params cpuattr;
  struct timespec now;
  unsigned long long now_ns;
  int rid;
  ...
  while (strstr(buf,"bye") == NULL){
    ...
    io_flag = 0;
    busy_timestamped(100,timestamps_nsa, MAX_TIMESTAMPS,&bufidxa);
  }
...
void *track_building_task(void *argp){
  struct zs_reserve_params cpuattr;
  struct timespec now;
  unsigned long long now_ns;
  int rid;
  ...
  while (strstr(buf,"bye") == NULL){
    ...
    io_flag = 0;
    busy_timestamped(100,timestamps_nsb, MAX_TIMESTAMPS,&bufidxb);
  }
...
void *traffic_warning_task(void *argp){
  struct zs_reserve_params cpuattr;
  struct timespec now;
  unsigned long long now_ns;
  int rid;
  ...
  while (strstr(buf,"bye") == NULL){
    ...
    busy_timestamped(100,timestamps_nsc, MAX_TIMESTAMPS,&bufidxc);
  }
